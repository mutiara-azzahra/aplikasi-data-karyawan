    ...
    <div class="container-xl">
        <div class="row row-cards">
            <div class="col-12">
                <div class="card">
                    <div class="col-12">
                        <form class="card" action="{{ route('departemen.update', $data->id) }}" method="POST">
                        @csrf
                            <div class="card-body">
                                <div class="mb-3">
                                    <label class="form-label required">Nama Departemen</label>
                                    <div>
                                        <input type="text" name="nama_departemen" class="form-control" value="{{ $data->nama_departemen }}">
                                    </div>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <div>
                                        <select name="id_head_departemen" id="select-tags" class="form-select">
                                            ...
                                            @foreach($head_departemen as $a)
                                                <option value="{{ $a->id }}" {{ old('head_departemen') == $a->id_head_departemen ? 'selected' : '' }}>{{ $a->nama_karyawan }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
